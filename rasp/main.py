...
def add_op(
    oriented_point: Shapes.OrientedPoint,
) -> bool:  # op stands for oriented point
...
while True:
    while (
        State.index_destination_point < State.index_last_point + 1
        and not State.game_finished
    ):  # while there are points left to go through and time is under treshold
        ...
        if (
            State.start_time != 0
            and Utils.get_current_date()["date_timespamp"] - State.start_time
            > State.time_to_return_to_home
        ):
            if not State.test:
        ...
        if (
            State.activate_print
            and (Utils.get_current_date()["date_timespamp"] - last_print) > 0.5
        ):
            last_print = Utils.get_current_date()["date_timespamp"]
