...
class TestRollingBasis:
    ...
    def test_calc_center_2(self):
        ...
        assert calc_center(a,b,c) == Shapes.OrientedPoint(4.39393887697407,1.822264208422694)        
    
    def test_curve_go_to_1(self):
        bot = RollingBasis(dummy=True)
        bot.odometrie = Shapes.OrientedPoint(5,5,0)
        assert bot.curve_go_to(Shapes.OrientedPoint(5,10,0), 10, 20, test=True) == Shapes.OrientedPoint(9.6875,7.5,0)
    ...
    def test_curve_go_to_2(self):
        ...
        assert bot.curve_go_to(Shapes.OrientedPoint(10,0,0), 5, 20, test=True) == Shapes.OrientedPoint(5,0,0)
