...
// tries to call the functions called by the rasp and send back an error message
// otherwise
void handle_callback(Com* com) {
    ...
    if (size > 0) {
        ...
        if (functions[msg[0]] !=
            0)  // verifies if the id of the function received by com is defined
        {
            functions[msg[0]](msg,
                              size);  // call the function by it's id and with
                                      // the parameters received by com
        } else if (msg[0] == NACK) {
            // send again the message that wasn't received by the rasp
            com->send_msg((byte*)&com->last_msg->msg, com->last_msg->size,
                          true);
        } else {
            ...
            com->send_msg(
                (byte*)&error_message,
                sizeof(msg_Unknown_Msg_Type));  // raise an error by sending 255
                                                // to the rasp if it tries to
                                                // send a message that isn't
                                                // defined by the teensy
        }
    }
}
...
void stepper_step(Bonezegei_A4988* stepper,
                  int steps,
                  bool forward,
                  byte pin_driver) {
...
void lcd_print(LiquidCrystal_I2C* lcd, String text) {
    lcd->clear();
    if (text.length() > 16) {
        String text1 = text.substring(
            0, 16);  // (16 is the number of columns of the lcd screen
        lcd->print(text1);
        lcd->setCursor(0, 1);
