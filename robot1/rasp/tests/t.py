...
class MLInvestmentStrategy(Indicateurs):
    def _init_(self, tickers, start_date, end_date, initial_capital=1000, lookback_period=6):
        super()._init_()
    ...
    def preprocess_data(self, data):
        ...
        feature_cols = ['Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume', 'Returns', 'SMA_20', 'RSI',
                        'Volume_Ratio']
        # Génération des colonnes en une seule opération
        lagged_features = {f'{col}lag{i}': data[col].shift(i) for i in range(1, self.lookback_period + 1) for col in
                           feature_cols}
    ...
    def execute_trades(self):
        ...
        for ticker in self.tickers:
            ...
            test_data = processed_data.loc[
                (processed_data.index >= self.start_date) & (processed_data.index <= self.end_date)]
            ...
            for date in test_data.index:
                feature_values = test_data.loc[date, selected_features]
                ...
                if missing_features:
                    print(f"⚠️ Certaines features sont absentes pour {ticker} à la date {date} : {missing_features}")
                    continue

                # Prédiction du modèle
                prediction = best_model.predict(feature_df)[0]
                open_price = test_data.loc[date, 'Open']
                close_price = test_data.loc[date, 'Close']

                # Vérification contre NaN ou division par zéro
                if pd.isna(open_price) or pd.isna(close_price) or open_price == 0:
                    print(f"⚠️ Données invalides pour {ticker} à la date {date}.")
                    continue
                ...
                print(
                    f"📅 {date}, {ticker} | Action: {action}, Daily Return: {daily_return_percentage:.2f}%, Capital: {capital:.2f}")
    ...
    def execute(self):
        ...
        for ticker in self.tickers:
            ...
            trade_summary.append(
                {"Ticker": ticker, "Buy_Trades": buy_trades, "Sell_Trades": sell_trades, "Model": best_model})
            ...
            try:
                ...
                print(
                    f"⚠️ La date d'investissement ou de fin est hors de la plage des données disponibles pour {ticker}.")
                continue
            ...
            portfolio_results[ticker] = {
                ...
                "volatilite_historique": float(volatilite_historique) if isinstance(volatilite_historique,
                                                                                    pd.Series) else volatilite_historique,
                ...
                "VaR Cornish-Fisher": float(var_cornish_fisher) if isinstance(var_cornish_fisher,
                                                                              pd.Series) else var_cornish_fisher,
                "CVaR Paramétrique": float(cvar_parametric) if isinstance(cvar_parametric,
                                                                          pd.Series) else cvar_parametric,
                "CVaR Historique": float(cvar_historical) if isinstance(cvar_historical,
                                                                        pd.Series) else cvar_historical,
                "CVaR Cornish-Fisher": float(cvar_cornish_fisher) if isinstance(cvar_cornish_fisher,
                                                                                pd.Series) else cvar_cornish_fisher,
        ...
        trade_summary_df["Model"] = trade_summary_df["Model"].astype(str)

        return {
            "gain_total": sum(result["gain_total"] for result in portfolio_results.values()),
            "pourcentage_gain_total": np.mean(
                [result["pourcentage_gain_total"] for result in portfolio_results.values()]),
            "performance_annualisee": performance_annualisee,
            "volatilite_historique": np.mean(
                [result["volatilite_historique"] for result in portfolio_results.values()]).item(),
            ...
            "trade_summary": trade_summary_df  # ✅ Nouveau DataFrame avec transactions et modèle par ticker
        }
