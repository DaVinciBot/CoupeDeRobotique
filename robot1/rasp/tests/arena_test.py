...
async def run_arena_test():
    ...
    arena.update(start, np.empty((0, 2)))
    ...
    while len(ally_path) > 3:
        ...
        arena.update(ally_position=ally_path[1],
            lidar_scan_polars=np.empty((0, 2)),
            optimized_update=True,)

        ax.clear()  # Clear the plot instead of closing it

        arena.visualize(plot=(ax, fig))
