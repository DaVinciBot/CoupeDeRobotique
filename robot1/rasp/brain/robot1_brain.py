    ...
    while True:
        try:
            ...
            if cmd != WSmsg():
                ...
                if cmd.msg == "Go_To":
                    ...
                    if MarsArena.enable_go_to(
                        starting_point=Point(rolling_basis.odometrie.x, rolling_basis.odometrie.y),
                        destination_point=Point(cmd.data[0], cmd.data[1])
                    ):
                        rolling_basis.Go_To(OrientedPoint(cmd.data[0], cmd.data[1], cmd.data[2]))
            ...
            await lidar_ws.sender.send(
                WSmsg(
                    msg="lidar_scan",
                    data=[[s.x, s.y] for s in lidar_scan]
                )
            )
            await odometer_ws.sender.send(
                WSmsg(
                    msg="odometer",
                    data=[rolling_basis.odometrie.x, rolling_basis.odometrie.y, rolling_basis.odometrie.theta]
                )
            )
