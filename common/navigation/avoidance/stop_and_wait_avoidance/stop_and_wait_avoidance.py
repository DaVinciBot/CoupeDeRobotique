...
class StopAndWaitAvoidance(BaseAvoidance[StopAndWaitAvoidanceParams]):
    ...
    def __init__(
        self, params: StopAndWaitAvoidanceParams, logger: Logger | None = None
    ) -> None:
    ...
    def handle(
        ...
        if (
            self._avoiding_start_time != 0.0
            and time.time() - self._avoiding_start_time > self.params.timeout
        ):
            self.state = AvoidanceState.ABORTED
            current_navigator_task.state = NavigatorTaskState.AVOIDING
            current_navigator_task.current_trajectory_plan_command = (
        ...
        if self._acs(ally_zone, enemy_zone):
            ...
            last_plan_path: BasePathPlannerPlanPathParams = (
            ...
            # Re-plan the path
            new_path: list[OrientedPoint] = (
                current_navigator_task.path_planner.plan_path(last_plan_path)
            )
