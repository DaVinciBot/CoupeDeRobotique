...
def generate_grid(width_cm, height_cm, chunk_size_cm, obstacle_ratio):
    ...
    grid = [
        [1 if random.random() > obstacle_ratio else 0 for _ in range(width)]
...
def update_dynamic_obstacles(grid, num_obstacles):
    ...
    return [
        (random.randint(0, len(grid[0]) - 1), random.randint(0, len(grid) - 1))
...
def py_path_run():
    ...
    grid = generate_grid(
        width_cm=300, height_cm=200, chunk_size_cm=20, obstacle_ratio=0.0
    )
    start = (1, 1)
    goal = (13, 8)
