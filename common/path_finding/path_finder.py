...
class PathFinder:
    ...
    def __path_to_oriented_path(self, path: list[GridNode]) -> list[OrientedPoint]:
        ...
        for i in range(len(path) - 1):
            current_node = GridNode(*self.grid_manager.get_grid_node_center(path[i]))
            next_node = GridNode(*self.grid_manager.get_grid_node_center(path[i + 1]))
    ...
    def __grid_coords_to_absolute_coords(self, node: GridNode) -> Point:
        ...
        x, y = self.grid_manager.get_grid_node_center(node)
        return Point(x * self.grid_manager.chunk_size, y * self.grid_manager.chunk_size)
