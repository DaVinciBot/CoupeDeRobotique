...
class ShowArena(BaseArena):
    def __init__(self,
        ...
        zones: list[BaseArenaZone] = [
            # Stuff zones first
            StuffZone(
                create_straight_rectangle(Point(2.5, 20), Point(7.5, 60))
            ),
            StuffZone(
                create_straight_rectangle(Point(2.5, 115), Point(7.5, 155))
            ),
            StuffZone(
                create_straight_rectangle(Point(57.5, 20), Point(97.5, 30))
            ),
            StuffZone(
                create_straight_rectangle(Point(90, 90), Point(130, 100))
            ),
            StuffZone(
            ...
            YellowReservedZone(
                create_straight_rectangle(Point(0, 0), Point(45, 15))
            ),
            YellowReservedZone(
                create_straight_rectangle(Point(0, 65), Point(45, 110))
            ),
            BlueReservedZone(
                create_straight_rectangle(Point(15, 165), Point(60, 200))
            ),
            BlueReservedZone(
        ...
        super().__init__(
            ...
            border_buffer=border_buffer, obstacle_buffer=obstacle_buffer,
            zones=zones, chunk_size=chunk_size
        )
