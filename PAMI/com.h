...
class Callback {
   public:
    ...
    static Callback* push_callback_to_array(Callback* old_ptr,
                                            const Callback& callback_to_push,
                                            unsigned short new_size);

    // Handle callbacks array
    static void handle_callback_array(Callback* callbacks_ptr,
...
class Server_Manager {
   private:
    ...
    // Tools
    String make_request(const String& msg,
                        const String& data,
                        bool use_millis_as_ts = true);
...
// Com class to manage the communication with the server, with only high-level
// methods
class Com {
   private:
