...
Callback* Callback::push_callback_to_array(Callback* old_ptr,
                                           const Callback& callback_to_push,
...
void Callback::handle_callback_array(Callback* callback_ptr,
                                     unsigned short nb_callbacks,
                                     const String& msg,
                                     String& data) {
...
// Private method(s)
// This method is called when a new event is received from the server through
// the WebSocket (connection, disconnection, new message)
    ...
    switch (type) {
        ...
        case WStype_TEXT:
            ...
            DeserializationError error =
                deserializeJson(json_message, String((char*)payload));
...
// Handle deserialization error
            ...
            */
            ...
            if (json_message.containsKey("sender") &&
                json_message.containsKey("msg") &&
                json_message.containsKey("data")) {
                // Get the msg and data from the message
                String msg = json_message["msg"];
                String data = json_message["data"];

                // Handle the message: if a callback is registered for this
                // message, call its associated function
                Callback::handle_callback_array(this->callbacks_ptr,
                                                this->nb_callbacks, msg, data);
            } else {
#ifdef WS_COM_DEBUG
...
// This method is used to create a request to send to the server, following the
// correct message format
String Server_Manager::make_request(const String& msg,
                                    const String& data,
                                    bool use_millis_as_ts) {
...
// This mehod is used to send a string to the server. This string must be a
// valid message ! This method should not be used directly, prefer using the
// make_request() method to prepare the message before
...
// Public method(s)
// This method is used to initialize the connection to the WiFi, then with the
// server
...
// Connect to WiFi
    ...
    // Connect to WS
    this->ws_client.begin(this->server_ip, this->server_port,
                          this->endpoint + String("?sender=") + this->username);
    this->ws_client.onEvent(
...
// This method is used to handle the connection with the server, check if new
// messages are received, and call the associated callback
...
// When this message is received, the associated function will be called with
// the message's data as parameter
void Server_Manager::add_callback(const String& msg,
                                  void (*function)(String data)) {
    this->nb_callbacks++;
    this->callbacks_ptr = Callback::push_callback_to_array(
        this->callbacks_ptr, Callback(msg, (*function)), this->nb_callbacks);
}

// This method is used to send a message to the server, it will create the
// message following the correct format before sending it
void Server_Manager::send(const String& msg,
                          const String& data,
                          bool use_millis_as_ts) {
    this->send_request(this->make_request(msg, data, use_millis_as_ts));
}
#pragma endregion
