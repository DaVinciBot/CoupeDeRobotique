...
void lidar_pami::begin(uint32_t baud) {
    ...
    if (_debug) Serial.println(F("lidar_pami: scan command sent"));
}
...
bool lidar_pami::obstacleAhead(uint16_t distanceMin) {
    if (!readFrame()) return false;   // no complete frame yet
    ...
    if (validCount == 0) return false;   // no valid points
    mean /= validCount;

    if (_debug) {
        Serial.print(F("Mean distance: ")); Serial.println(mean);
    }
