...
// Start update process
        ...
        if (!Update.begin(update_size,
                          mode == OTA_MODE_FILESYSTEM ? U_FS : U_FLASH)) {
    ...
    _server->on("/ota/start", HTTP_GET, [&]() {
        ...
        if (_server->hasArg("hash")) {
            ...
            if (!Update.setMD5(hash.c_str())) {
                ELEGANTOTA_DEBUG_MSG("ERROR: MD5 hash not valid\n");
                return _server->send(400, "text/plain",
                                     "MD5 parameter invalid");
            }
        }
        ...
        [&](AsyncWebServerRequest* request, String filename, size_t index,
            ...
            if (len) {
                ...
                if (progressUpdateCallback != NULL)
                    progressUpdateCallback(_current_progress_size,
    ...
    _server->on(
        ...
        [&]() {
            ...
            if (upload.status == UPLOAD_FILE_START) {
                ...
                if (_authenticate &&
                    !_server->authenticate(_username.c_str(),
                                           _password.c_str())) {
                    ELEGANTOTA_DEBUG_MSG(
                        "Authentication Failed on UPLOAD_FILE_START\n");
                    return;
                }
                ...
                if (Update.end(true)) {
                    ELEGANTOTA_DEBUG_MSG(String("Update Success: " +
                ...
                ELEGANTOTA_DEBUG_MSG(
                    String("Update Failed Unexpectedly (likely broken "
                           "connection): status=" +
...
void ElegantOTAClass::onProgress(
    std::function<void(size_t current, size_t final)> callable) {
